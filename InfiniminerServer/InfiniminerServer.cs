...
namespace Infiniminer
{
    public class InfiniminerServer
    {
        ...
        public bool Start()
        {
            ...
            //netServer.se(NetMessageType.ConnectionApproval, true);
            ...
            //NetConnection msgType.SenderConnection = netServer.GetConnection(netCo);// = new NetConnection();
            
            while (keepRunning)
            {
                var ret = netServer.ReadMessage();
                NetIncomingMessage msgType;
                // Process any messages that are here.
                while ((msgType=netServer.ReadMessage())!=null)
                {
                    try
                    {
                        switch (msgType.MessageType)
                        {
                            case NetIncomingMessageType.ConnectionApproval:
                                {
                                    Player newPlayer = new Player(msgType.SenderConnection, null);
                                    ...
                                    {
                                        msgType.SenderConnection.Deny("VER;" + Defines.INFINIMINER_VERSION);
                                    }
                                    else if (banList.Contains(newPlayer.IP))
                                    {
                                        msgType.SenderConnection.Deny("BAN;");
                                    }/*
                                else if (playerList.Count == maxPlayers)
                                {
                                    msgType.SenderConnection.Disapprove("FULL;");
                                }*/
                                    else
                                    {
                                        if (admins.ContainsKey(newPlayer.IP))
                                            newPlayer.admin = admins[newPlayer.IP];
                                        playerList[msgType.SenderConnection] = newPlayer;
                                        //Check if we should compress the map for the client
                                        try
                                        {
                                            bool compression = msgBuffer.ReadBoolean();
                                            if (compression)
                                                playerList[msgType.SenderConnection].compression = true;
                                        }
                                        catch { }
                                        toGreet.Add(msgType.SenderConnection);
                                        this.netServer.SanityCheck(msgType.SenderConnection);
                                        msgType.SenderConnection.Approve();
                                        PublicServerListUpdate(true);
                                    }
                                }
                                break;
                            ...
                            case NetIncomingMessageType.StatusChanged:
                                {
                                    if (!this.playerList.ContainsKey(msgType.SenderConnection))
                                    {
                                        break;
                                    }

                                    Player player = playerList[msgType.SenderConnection];

                                    if (msgType.SenderConnection.Status == NetConnectionStatus.Connected)
                                    {
                                        ConsoleWrite("CONNECT: " + playerList[msgType.SenderConnection].Handle + " ( " + playerList[msgType.SenderConnection].IP + " )");
                                        SendCurrentMap(msgType.SenderConnection);
                                        SendPlayerJoined(player);
                                        PublicServerListUpdate();
                                    }

                                    else if (msgType.SenderConnection.Status == NetConnectionStatus.Disconnected)
                                    {
                                        ConsoleWrite("DISCONNECT: " + playerList[msgType.SenderConnection].Handle);
                                        SendPlayerLeft(player, player.Kicked ? "WAS KICKED FROM THE GAME!" : "HAS ABANDONED THEIR DUTIES!");
                                        if (playerList.ContainsKey(msgType.SenderConnection))
                                            playerList.Remove(msgType.SenderConnection);
                                        PublicServerListUpdate();
                                    }
                                }
                                break;
                            ...
                            case NetIncomingMessageType.Data:
                                {
                                    ...
                                    switch (dataType)
                                    {
                                        case InfiniminerMessage.ChatMessage:
                                            {
                                                ...
                                                if (!ProcessCommand(chatString, GetAdmin(playerList[msgType.SenderConnection].IP), playerList[msgType.SenderConnection]))
                                                {
                                        ...
                                        case InfiniminerMessage.PlayerAlive:
                                            {
                                                if (toGreet.Contains(msgType.SenderConnection))
                                                {
                                                    string greeting = varGetS("greeter");
                                                    greeting = greeting.Replace("[name]", playerList[msgType.SenderConnection].Handle);
                                                    if (greeting != "")
                                                    {
                                                        ...
                                                        netServer.SendMessage(greetBuffer, msgType.SenderConnection, NetDeliveryMethod.ReliableOrdered);
                                                    }
                                                    toGreet.Remove(msgType.SenderConnection);
                                                }
